    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
                <h4 class="page-title">{{ isset($order) ? 'Cập nhật Đơn hàng' : 'Thêm Đơn hàng' }}</h4>
                <div class="ms-auto text-end">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">Admin</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{ isset($order) ? 'Cập nhật Đơn hàng' : 'Thêm Đơn hàng' }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <form class="form-horizontal" method="POST" action="{{ isset($order) ? route('order.update', $order->id) : route('order.store') }}">
                        ...
                        <div class="card-body">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="khach_hang_id" name="khach_hang_id" required>
                                        ...
                                        @foreach($khachHangs as $khachHang)
                                            <option value="{{ $khachHang->id }}" {{ isset($order) && $order->khach_hang_id == $khachHang->id ? 'selected' : '' }}>{{ $khachHang->ten }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="khach_hang_id" name="trang_thai" required>
                                        ...
                                        <option value="Đang xữ lý" {{ $order->trang_thai == 'Đang xữ lý' ? 'selected' : '' }}>Đang xữ lý</option>
                                        <option value="Đang giao hàng" {{ $order->trang_thai == 'Đang giao hàng' ? 'selected' : '' }}>Đang giao hàng</option>
                                        <option value="Đã hoàn thành" {{ $order->trang_thai == 'Đã hoàn thành' ? 'selected' : '' }}>Đã hoàn thành</option>
                                    </select>
                                </div>
                            </div>
                            @else
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="khach_hang_id" name="trang_thai_thanh_toan" required>
                                        ...
                                        <option value="Đã thanh toán qua ngân hàng" {{ $order->trang_thai_thanh_toan == 'Đã thanh toán qua ngân hàng' ? 'selected' : '' }}>Đã thanh toán qua ngân hàng</option>
                                        <option value="Đã thanh toán bằng tiền mặt" {{ $order->trang_thai_thanh_toan == 'Đã thanh toán bằng tiền mặt' ? 'selected' : '' }}>Đã thanh toán bằng tiền mặt</option>
                                    </select>
                                </div>
                            </div>
                            @else
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="khach_hang_id" name="trang_thai_thanh_toan" required>
                                        ...
                                        <option value="Đã thanh toán qua ngân hàng" >Đã thanh toán qua ngân hàng</option>
                                        <option value="Đã thanh toán bằng tiền mặt" >Đã thanh toán bằng tiền mặt</option>
                                    </select>
                                </div>
                            </div>
                            @endif
                            ...
                            <div id="product-list">
                                    ...
                                    @foreach($order->chiTietDonHangs as $key => $chiTiet)
                                        <div class="product-item" data-index="{{ $key }}">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-6">
                                                    <select class="form-control san-pham" id="san_pham_id_{{ $key }}" name="san_pham_id[]" onchange="updatePrice(this, {{ $key }})">
                                                        <option value="">Chọn sản phẩm</option>
                                                        @foreach($sanPhams as $sanPham)
                                                            <option value="{{ $sanPham->id }}" data-price="{{ $sanPham->gia }}" {{ $chiTiet->san_pham_id == $sanPham->id ? 'selected' : '' }}>{{ $sanPham->ten }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="number" class="form-control so-luong" name="so_luong[]" id="so_luong_{{ $key }}" value="{{ $chiTiet->so_luong }}" oninput="updateTotal()" />
                                                </div>
                                            </div>
                                ...
                                @else
                                    <div class="product-item" data-index="0">
                                        <div class="form-group row">
                                            <label for="san_pham_id_0" class="col-sm-3 text-end control-label col-form-label">Sản phẩm</label>
                                            <div class="col-sm-6">
                                                <select class="form-control san-pham" id="san_pham_id_0" name="san_pham_id[]" onchange="updatePrice(this, 0)">
                                                    <option value="">Chọn sản phẩm</option>
                                                    @foreach($sanPhams as $sanPham)
                                                        <option value="{{ $sanPham->id }}" data-price="{{ $sanPham->gia }}">{{ $sanPham->ten }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-sm-3">
                                                <input type="number" class="form-control so-luong" name="so_luong[]" id="so_luong_0" value="1" oninput="updateTotal()" />
                                            </div>
                                        </div>

                                        <div class="form-group row">
                                            <label for="gia_0" class="col-sm-3 text-end control-label col-form-label">Giá sản phẩm</label>
                                            <div class="col-sm-6">
                                                <input type="text" class="form-control gia" id="gia_0" readonly />
                                            </div>
                                        </div>
                            ...
                            <button type="button" class="btn btn-danger" onclick="removeProduct()">Xóa sản phẩm</button>

                            <!-- Tổng tiền -->
                            <div class="form-group row">
                                <label for="tong_tien" class="col-sm-3 text-end control-label col-form-label">Tổng tiền</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" id="tong_tien" name="tong_tien" readonly />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-top">
                            <div class="card-body">
                                <button type="submit" class="btn btn-primary">{{ isset($order) ? 'Cập nhật' : 'Thêm mới' }}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    function updateTotal() {
        ...
        document.querySelectorAll('.product-item').forEach(item => {
            const price = parseFloat(item.querySelector('.san-pham').selectedOptions[0].getAttribute('data-price')) || 0;
